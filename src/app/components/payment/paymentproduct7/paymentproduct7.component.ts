...
@Component({
  selector: 'app-paymentproduct7',
  templateUrl: './paymentproduct7.component.html',
  styleUrls: ['./paymentproduct7.component.css']
})
export class Paymentproduct7Component implements OnInit {
    ...
    if ( isAllawableExt == false ) {
      ...
      this._attachmentService.upload( formData ).subscribe( response => {
        if ( response.Success ) {
          ...
          this.model.Type = PaymentType.ProductCertificate;
        }
      }, error => {
        this._alertService.error( "Maximum file size is 5 MB" );
